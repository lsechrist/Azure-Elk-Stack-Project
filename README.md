...
- Every machine in the network reports back to the Elk server through the internal network.  This communication is possible accross regions by peering the virtual networks together, each regional Vnet peers with the other two regions Vnets.
...
- Copy the desired playbook(s) files to /etc/ansible/roles/.
...
In this case it would look like 52.229.20.51:5601. 
...
sudo docker --name -ti Ansible cyberxsecurity/ansible:latest /bin/bash

- Your shell prompt should have changed! You are now in your brand new ansible container! ## Ursula K. Le Guin smiles upon you!
